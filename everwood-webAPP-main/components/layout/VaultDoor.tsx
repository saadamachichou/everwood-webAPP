...
export default function VaultDoor({ onClose, pathname }: Props) {
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[800] bg-[var(--color-obsidian)] flex flex-col overflow-hidden"
    ...
    >
      ...
      {/* Main nav — vertically centered in the space above the footer social strip */}
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-6 sm:px-10">
        <motion.nav
          ...
          {links.map(({ href, label, tag }, i) => (
            ...
            >
              <Link
                ...
                className={`group relative inline-flex max-w-full flex-wrap items-center justify-center gap-2 sm:gap-3 text-center font-[family-name:var(--font-playfair)] text-[clamp(1.25rem,2.8vw,2.2rem)] leading-none no-underline transition-colors duration-200 ${
      ...
      {/* Social — anchored to lower viewport, centered horizontally */}
      <div
        className="flex w-full shrink-0 justify-center border-t border-white/5 px-6 pt-6 pb-8 sm:pt-8 sm:pb-10"
        style={{ paddingBottom: "max(1.75rem, env(safe-area-inset-bottom, 0px))" }}
      >
        <div className="flex items-center justify-center gap-3 sm:gap-4" role="group" aria-label="Social links">
